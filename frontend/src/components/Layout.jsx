...
const Layout = ({ onLogout, user }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-gray-900 to-orange-900 text-gray-100">
      ...
      <div className='ml-0 xl:ml-64 lg:ml-64 md:ml-16 pt-16 p-6 transition-all duration-300'>
        <div className='grid grid-cols-1 xl:grid-cols-3 gap-6'>
          ...
          <div className="xl:col-span-1 space-y-6">
            <div className="glass-dark rounded-2xl p-6 shadow-xl border border-purple-500/20 animate-slideUp neon-glow">
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                <StatCard title='Total Tasks' value={stats.totalCount} icon={<Circle className='w-5 h-5 text-purple-400' />} />
                ...
                <StatCard title='Success Rate' value={`${stats.completionPercentage}%`} icon={<Zap className='w-5 h-5 text-pink-400' />} />
              </div>

              <div className="glass p-4 rounded-xl border border-purple-400/20">
                <div className="flex items-center justify-between text-gray-300 mb-3">
                  <span className='text-sm font-semibold flex items-center gap-2'>
                    <div className="w-3 h-3 rounded-full bg-gradient-to-r from-purple-400 to-orange-400 animate-glow" />
                    Progress Overview
                  </span>
                ...
                <div className='relative pt-2'>
                  <div className="h-4 bg-gray-800/50 rounded-full overflow-hidden border border-gray-700/50">
                    <div className="h-full bg-gradient-to-r from-purple-500 via-pink-400 to-orange-500 transition-all duration-1000 ease-out shadow-lg neon-glow"
                      style={{ width: `${stats.completionPercentage}%` }} />
                  </div>
                  <div className="absolute inset-0 rounded-full bg-gradient-to-r from-purple-500/20 to-orange-500/20 animate-pulse" />
                </div>
              </div>
            </div>

            <div className="glass-dark rounded-2xl p-6 shadow-xl border border-purple-500/20 animate-slideUp orange-glow">
              ...
              <div className="space-y-4">
                {tasks.slice(0, 3).map((task, index) => (
                  <div key={task._id || task.id} className='group flex items-center justify-between p-4 glass rounded-xl border border-purple-400/20 hover:border-orange-400/40 transition-all duration-300 hover:scale-[1.02] animate-fadeIn' style={{animationDelay: `${index * 100}ms`}}>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-white truncate group-hover:text-purple-300 transition-colors">{task.title}</p>
                ...
                {tasks.length === 0 && (
                  <div className="text-center py-8 px-4">
                    <div className="w-20 h-20 mx-auto mb-4 rounded-full bg-gradient-to-br from-purple-500/20 to-orange-500/20 flex items-center justify-center animate-float">
